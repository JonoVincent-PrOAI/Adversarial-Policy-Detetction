...
class Adversarial_Detection_Dataset(Dataset):
    ...
    file_list (OPTIONAL): Should be output of os.listdir().If a file_list is passed instead of using all files in 
                          dir_path, only the files in file_list will be used.
    '''
    ...
    '''
    ---description---
    Returns item from dataset when a valid index.
    ---input---
    idx: int: the index of the dataset item you want to return
    ---output---
    idx: int: the index of returned item
    sample['layer activation']: pytorch.tensor: adv_det_model input 
    ...
    '''
    ---description---
    Utility for retireve whole sample not just inputs and labels.
    ---input---
    idx: int: index of sample you want to retrieve
    ---output---
    ...
    '''
    ---description---
    Given an idx, returns the file_list index of the game that sample is from.
    ---input---
    idx: int: idx of sample, you want the game index for
    ---output---
